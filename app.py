...
def get_candy():
    ...
    keys = ["candyId", "candyName", "category"]
    response = []
    if (type(result) == list):
        for candy in result:
...
# Clients can DELETE candy by giving a name
...
def delete_candy():
    ...
    candyName
    """
    required_data = ['candyName']
    check_result = check_data(request.json, required_data)
    if check_result != None:
        return check_result
    candy_name = request.json.get('candyName')
    result = run_statement("CALL delete_candy(?)", [candy_name])
    if (type(result) == list):
        if result[0][0] == 1:
            return make_response(jsonify(f"Successfully deleted candy {candy_name}."), 200)
        elif result[0][0] == 0:
            return make_response(jsonify(f"Delete unsuccessful for candy {candy_name}, check candy name."), 400)
    else:
        "There has been an unexpected error."
